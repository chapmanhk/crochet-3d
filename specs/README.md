...
| Chain length must be within allowed bounds | `tests/engine/Pattern.test.ts`, `tests/store/patternStore.test.ts` |
...
| Cannot add single crochet on the foundation row | `tests/store/patternStore.test.ts`, `e2e/app.spec.ts` |
| Row cannot exceed foundation length | `tests/store/patternStore.test.ts` |
...
| Cannot start a new row with no stitches on the current row | `tests/store/patternStore.test.ts` |
